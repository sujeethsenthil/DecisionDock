...
import { fmtLatencyExact } from "@/lib/format";

function latencyZoneLabel(n: number): string {
  return n <= 3 ? "Adequate" : n <= 4 ? "Perception threshold" : "Physics-limited";
}
...
export function SliderPanel({ current, target, onTargetChange, tokens: T }: Props) {
  ...
  const label = latencyZoneLabel(target);
  ...
  return (
    <div style={{ background: C.white, borderRadius: 16, border: `1px solid ${C.border}`, boxShadow: "0 1px 3px rgba(0,0,0,0.04)", padding: T.rPad }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 6 }}>
        <span style={{ fontSize: T.rNumFs, fontWeight: 300, color, fontFamily: "'Inter',sans-serif", letterSpacing: "-0.04em" }}>{fmtLatencyExact(target)}</span>
      </div>
      <input
        type="range" min={current} max={6} step={0.1} value={target}
        onChange={(e) => onTargetChange(parseFloat(e.target.value))}
        aria-label="Set target latency"
        style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 3 }}>
        <span style={{ fontSize: T.rLabelFs, color: C.subtle }}>{fmtLatencyExact(current)}</span>
        <span style={{ fontSize: T.rLabelFs, color: C.subtle }}>7.8ms</span>
      </div>
    </div>
  );
}
