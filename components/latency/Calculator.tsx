...
export function Calculator({ onChartClick, onSliderDrag, chartRef, sliderRef, upgradeRef, bottomRef }: Props) {
  ...
  useEffect(() => {
    Analytics.domainViewed("latency");
  }, []);
  ...
  const handleSetCurrent = useCallback((level: number) => {
    ...
    Analytics.chartClicked("latency", clamped, cfg.levelLabel(clamped));
    onChartClick?.();
  ...
  const handleSliderChange = useCallback((v: number) => {
    ...
    if (!draggedRef.current) {
      ...
      Analytics.sliderDragged("latency", v, cfg.levelLabel(v), cfg.cost(v));
    }
    onSliderDrag?.();
