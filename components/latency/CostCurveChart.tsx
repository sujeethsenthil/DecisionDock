...
import { fc, fmtLatencyTick, fmtLatencyExact, fmtMs } from "@/lib/format";
import { latencyCost, latencyMs, latencyPerception } from "@/lib/models/latency";
...
function ChartTooltip({ active, payload }: any) {
  ...
  return (
    <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 14px", boxShadow: "0 4px 16px rgba(0,0,0,0.08)", fontSize: 13 }}>
      <div style={{ fontWeight: 600, color: C.navy }}>{fmtMs(latencyMs(pt.x))} p99</div>
...
export function CostCurveChart({ data, current, target, isUpgrade, onSetCurrent, tokens: T }: Props) {
  ...
  const bC = latencyCost(current);
  const bMs = latencyMs(current);
  const tC = latencyCost(target);
  ...
  return (
    <div style={{ background: C.white, borderRadius: 16, border: `1px solid ${C.border}`, boxShadow: "0 1px 3px rgba(0,0,0,0.04)", padding: 0, overflow: "hidden", display: "flex", flexDirection: "column", minHeight: T.chartMinH }}>
      <div style={{ padding: `10px ${T.gap + 6}px`, borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <span style={{ fontSize: T.chartTFs, fontWeight: 700, color: C.navy }}>What speed actually costs</span>
          <span style={{ fontSize: T.chartSFs, color: C.subtle, marginLeft: 8 }}>infrastructure spend by p99 latency target</span>
        </div>
        <span style={{ fontSize: T.srcFs, color: C.subtle, fontStyle: "italic" }}>Web application benchmark</span>
      </div>
      <div ref={chartRef} onClick={handleClick} style={{ flex: 1, padding: "4px 8px 0 0", minHeight: 0, position: "relative", cursor: "crosshair" }}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 24, right: 20, left: 12, bottom: 28 }}>
            ...
            <XAxis dataKey="x" type="number" domain={[2, 6]} ticks={[2, 3, 4, 5, 6]} tickFormatter={fmtLatencyTick} tick={{ fontSize: 11, fill: C.subtle }} axisLine={{ stroke: C.border }} tickLine={false} label={{ value: "P99 Latency Target", position: "insideBottom", offset: -8, fill: C.subtle, fontSize: 11 }} />
            ...
            <Area type="monotone" dataKey="cost" stroke={C.blue} strokeWidth={2.5} fill="url(#lg)" isAnimationActive={animate} animationDuration={350} animationEasing="ease-out" />
            <Tooltip content={<ChartTooltip />} />
          </AreaChart>
        </ResponsiveContainer>

        <TrackingLabel nines={current} color={C.blue} marginL={T.chartMarginL} marginR={T.chartMarginR}>
          <div style={{ fontSize: 9, color: C.blue, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>Your latency · {fmtMs(bMs)}</div>
          ...
          <div style={{ fontSize: 9, color: C.subtle }}>{latencyPerception(bMs)}</div>
        </TrackingLabel>

        {isUpgrade && (
          <TrackingLabel nines={target} color={color} marginL={T.chartMarginL} marginR={T.chartMarginR}>
            <div style={{ fontSize: 9, color, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>Considering · {fmtMs(tMs)}</div>
            ...
            <div style={{ fontSize: 9, color: C.subtle }}>{latencyPerception(tMs)}</div>
          </TrackingLabel>
        )}
      </div>
