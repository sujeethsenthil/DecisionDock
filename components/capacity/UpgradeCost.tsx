...
export function UpgradeCost({ current, target, isUpgrade, tokens: T }: Props) {
  ...
  const nextSlow = peakSlowdown(current) - peakSlowdown(nextLevel);
      ...
      {!isUpgrade ? (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
          <div style={{ background: C.bS, borderRadius: 10, padding: T.upgPad, marginBottom: g }}>
            <div style={{ fontSize: T.upgLFs, color: C.blue, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 4 }}>You&apos;re here · {fmtBuffer(bufferMultiplier(current))}</div>
            ...
            <div style={{ fontSize: T.upgLFs + 1, color: C.subtle, marginTop: 3 }}>{fmtSlowdown(bSlow)} slower under peak load</div>
          </div>
          <div style={{ background: C.light, borderRadius: 10, padding: T.upgPad }}>
            ...
            <div style={{ fontSize: T.botSFs || 11, color: C.body, lineHeight: 1.5 }}>Reaching <strong style={{ color: C.amber }}>{fmtCapacityExact(nextLevel)}</strong> buffer would cost ~<strong>{fc(nextCost)}/yr</strong> — a {Math.round(nextCost / bC)}× increase. Peak slowdown drops by {fmtSlowdown(nextSlow)}.</div>
          </div>
          <div style={{ marginTop: g, fontSize: T.upgLFs + 1, color: C.subtle, textAlign: "center", lineHeight: 1.4 }}>
            Drag the slider to see what more headroom costs →
          </div>
        </div>
      ) : (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
