...
import { fc, fmtCapacityTick, fmtBuffer, fmtSlowdown } from "@/lib/format";
...
function ChartTooltip({ active, payload }: any) {
  ...
  return (
    <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 14px", boxShadow: "0 4px 16px rgba(0,0,0,0.08)", fontSize: 13 }}>
      <div style={{ fontWeight: 600, color: C.navy }}>{fmtBuffer(bufferMultiplier(pt.x))} buffer</div>
...
export function CostCurveChart({ data, current, target, isUpgrade, onSetCurrent, tokens: T }: Props) {
  ...
  const bC = capacityCost(current);
  const bBuf = bufferMultiplier(current);
  const bSlow = peakSlowdown(current);
  const tC = capacityCost(target);
  const tBuf = bufferMultiplier(target);
  ...
  return (
    <div style={{ background: C.white, borderRadius: 16, border: `1px solid ${C.border}`, boxShadow: "0 1px 3px rgba(0,0,0,0.04)", padding: 0, overflow: "hidden", display: "flex", flexDirection: "column", minHeight: T.chartMinH }}>
      <div style={{ padding: `10px ${T.gap + 6}px`, borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <span style={{ fontSize: T.chartTFs, fontWeight: 700, color: C.navy }}>What capacity buffer actually costs</span>
          <span style={{ fontSize: T.chartSFs, color: C.subtle, marginLeft: 8 }}>infrastructure spend by peak headroom level</span>
        </div>
        <span style={{ fontSize: T.srcFs, color: C.subtle, fontStyle: "italic" }}>Cloud auto-scaling benchmark</span>
      </div>
      <div ref={chartRef} onClick={handleClick} style={{ flex: 1, padding: "4px 8px 0 0", minHeight: 0, position: "relative", cursor: "crosshair" }}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 24, right: 20, left: 12, bottom: 28 }}>
            ...
            <XAxis dataKey="x" type="number" domain={[2, 6]} ticks={[2, 3, 4, 5, 6]} tickFormatter={fmtCapacityTick} tick={{ fontSize: 11, fill: C.subtle }} axisLine={{ stroke: C.border }} tickLine={false} label={{ value: "Peak Capacity Buffer", position: "insideBottom", offset: -8, fill: C.subtle, fontSize: 11 }} />
            <YAxis tickFormatter={fc} tick={{ fontSize: 11, fill: C.subtle }} axisLine={false} tickLine={false} width={54} label={{ value: "Annual Infra Cost", angle: -90, position: "insideLeft", offset: 4, fill: C.subtle, fontSize: 11 }} />
            ...
            <Area type="monotone" dataKey="cost" stroke={C.blue} strokeWidth={2.5} fill="url(#capg)" isAnimationActive={animate} animationDuration={350} animationEasing="ease-out" />
            <Tooltip content={<ChartTooltip />} />
          </AreaChart>
        </ResponsiveContainer>

        <TrackingLabel nines={current} color={C.blue} marginL={T.chartMarginL} marginR={T.chartMarginR}>
          <div style={{ fontSize: 9, color: C.blue, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>Your buffer · {fmtBuffer(bBuf)}</div>
          ...
          <div style={{ fontSize: 9, color: C.subtle }}>{fmtSlowdown(bSlow)} slower at peak</div>
        </TrackingLabel>

        {isUpgrade && (
          <TrackingLabel nines={target} color={color} marginL={T.chartMarginL} marginR={T.chartMarginR}>
            <div style={{ fontSize: 9, color, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>Considering · {fmtBuffer(tBuf)}</div>
            ...
            <div style={{ fontSize: 9, color: C.subtle }}>{fmtSlowdown(tSlow)} slower at peak</div>
          </TrackingLabel>
        )}
      </div>
      <div style={{ padding: `5px ${T.gap + 6}px 6px`, borderTop: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 6 }}>
        <span style={{ fontSize: T.srcFs, color: C.subtle }}>Click the chart to set where you are today · Sources: Kleinrock, Google SRE Book</span>
        <span style={{ fontSize: T.srcFs, color: C.subtle }}>R = S/(1−ρ) — the queuing hockey stick</span>
      </div>
    </div>
  );
}
