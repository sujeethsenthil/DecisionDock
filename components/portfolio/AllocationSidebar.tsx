...
export function AllocationSidebar({ budget, targetX, tokens: T }: Props) {
  // Compute per-domain costs and totals
  ...
  let hint = "";
  if (isOver) {
    hint = `${biggestLabel} is your biggest driver — its ROI curve has ${zoneDesc}.`;
  } else if (isZero) {
    hint = "All domains funded within budget.";
  } else {
    hint = "Distribute remaining funds to your highest-priority domain.";
  }
    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "column" }}>
        {DOMAIN_ORDER.map((k, i) => {
          ...
          const isLastRow = i === DOMAIN_ORDER.length - 1;
          ...
          return (
            ...
            >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6, flex: 1, minWidth: 0 }}>
                ...
                />
                <span style={{ fontSize: 11, color: C.med, fontWeight: 500 }}>
                  {cfg.label}
                </span>
              </div>
      ...
      <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 12 }}>
        ...
        <div style={{ marginBottom: 4 }}>
          ...
          >
            {isOver ? "−" : ""}<AnimatedCounter value={Math.abs(pool)} color={poolColor} size={28} />
          </span>
        </div>
        ...
        <div style={{ fontSize: 11, color: C.med, marginBottom: 8 }}>
          {isOver
            ? `Over budget by ${fcFull(Math.abs(pool))}`
            : isZero
