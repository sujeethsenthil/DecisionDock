...
export function UpgradeCost({ current, target, isUpgrade, tokens: T }: Props) {
  ...
  const nextCfr = changeFailRate(current) - changeFailRate(nextLevel);
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: g }}>
            ...
            <div style={{ flex: 1, background: C.eS, borderRadius: 10, padding: T.upgPad, textAlign: "center" }}>
              <div style={{ fontSize: T.upgLFs, color: C.subtle, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 3 }}>Failure rate you&apos;d eliminate</div>
              <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: T.upgNFs + 2, fontWeight: 700, color: C.emerald }}>{dCfr.toFixed(1)} pts</div>
              <div style={{ fontSize: T.upgLFs, color: C.subtle, marginTop: 2 }}>fewer failures</div>
            </div>
          </div>
